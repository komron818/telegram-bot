...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer("Добро пожаловать в Oson hayot! Выберите товар:", reply_markup=kb.as_markup())
...
async def select_quantity(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Выберите следующий товар или перейдите к чеку:", reply_markup=kb.as_markup())
    await state.set_state(OrderState.choosing)
...
<b>Общая сумма: {total:,} сум</b>"
    ...
    await callback.message.answer("Пожалуйста, отправьте ваш номер и геолокацию перед подтверждением:", reply_markup=contact_kb)
...
async def receive_contact_location(message: Message, state: FSMContext):
    ...
    if "phone" in user_contacts[user_id] and "location" in user_contacts[user_id]:
        ...
        await message.answer("Теперь вы можете подтвердить заказ:", reply_markup=kb.as_markup())
